...
def main():
    ...
    if file_upload is not None:
        ...
        X_list = [dv.transform(row.to_dict()) for _, row in data.iterrows()]
        X = np.vstack(X_list)  # Combine the transformed rows into a single array
        y_pred = model.predict_proba(X)[:, 1]  # Predict probabilities for churn (class 1)
        churn = y_pred >= 0.5
        data['Churn'] = churn.astype(int)  # Add the 'Churn' column to the DataFrame
        st.write(data)
